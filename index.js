...
async function fetchAQIData(lat, lon) {
    let fetchAQIData = await fetch(`https://api.openweathermap.org/data/2.5/air_pollution?lat=${lat}&lon=${lon}&appid=9572c3d8f54774e1c78d963a55221763`)
...
async function nextFiveDays(lat, lon) {
    ...
    const apiUrl = `https://api.openweathermap.org/data/2.5/forecast?lat=17.69&lon=83.2093&appid=9572c3d8f54774e1c78d963a55221763&units=metric`;
    try {
            ...
            forecastHtml += `
                        <div class="forecastRow d-flex align-items-center justify-content-between">
                            <div class="d-flex gap-1 align-items-center">
                                <img src="https://i.ibb.co/kV90N0j2/cloud.png" alt="cloud" border="0" width="35px">
...
async function todayTemps(lat, lon) {
    const apiUrl = `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=9572c3d8f54774e1c78d963a55221763&units=metric`;
    ...
    try {
        ...
        let selectedHours = data.list.slice(0, 6); // Get first 6 hourly forecasts
        ...
        selectedHours.forEach(item => {
            ...
            todayHtml += `
                        <div class="todayTemp">
                            ...
                            <img src="https://i.ibb.co/5gCg0D4X/cloudy.png" alt="cloudy" border="0" width="35px">
                            <h5>${temp}&deg;C</h5>
                        </div>
                    `;
        });
...
async function fetchData() {
    ...
    let requestData = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=9572c3d8f54774e1c78d963a55221763&units=metric`)
